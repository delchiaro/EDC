 ...
 *
 * \if DeveloperDocs
 *   \brief Plugin file appender - writes log messages to a local file using PTH functions
 * \endif
 */
 ...
 */
static int filePthOpen( sAppender *pApp );
static int filePthClose( sAppender *pApp );
static int filePthLog( sAppender *pApp, unsigned int level, char *message );
static int filePthCheck( char *params );
...
static sAppenderPlugin     structFile = {
       "file:", "%s", /* 1, */ filePthOpen, filePthClose, filePthLog, filePthCheck, NULL
};
...
static int filePthLog( sAppender *pApp, unsigned int level, char *message )
{
    ...
    if( pApp->channel != NULL ) {
        pth_write( fileno( pApp->channel ), message, strlen( message ));
        pth_write( fileno( pApp->channel ), newline, strlen( newline ));
        fflush( pApp->channel );
        return( ZLOG_NO_ERROR );
    }
